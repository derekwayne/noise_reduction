...
\newline
```{r fig.width=5, fig.height=5, fig.cap="Original Noisy Image"}
library(ggplot2)
...
image(boats, axes=F, col=grey(seq(0,1,length=256)), xlab="Original")
image(soft, axes=F, col=grey(seq(0,1,length=256)), xlab="Soft Threshold: lam=7")
```
```{r, fig.align="center"}
par(mfrow=c(1,2))
hard <- sub.im.transform(boats, 32, Th.type = "hard", thresh = 6)
image(boats, axes=F, col=grey(seq(0,1,length=256)), xlab="Original")
image(hard, axes=F, col=grey(seq(0,1,length=256)), xlab="Hard Threshold: lam=6")
```
```{r, fig.align="center"}
par(mfrow=c(1,2))
soft2 <- sub.im.transform(boats, 8, Th.type = "soft", thresh = 0.8)
image(boats, axes=F, col=grey(seq(0,1,length=256)), xlab="Original")
image(soft2, axes=F, col=grey(seq(0,1,length=256)), xlab="Soft Threshold: lam=0.8")
```
